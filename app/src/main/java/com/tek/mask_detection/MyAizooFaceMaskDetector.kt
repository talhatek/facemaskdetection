...
class MyAizooFaceMaskDetector(
    ...
    private val caffeeModelPath =
        assetToAppStoragePath("aizoo/face_mask_detection.caffemodel", context)
    ...
    override fun recognize(_picture: Bitmap): List<FaceMaskDetector.DetectedFace> {
        val picture = if (isTablet)
            _picture
        else {
            if (facing == CameraSelector.LENS_FACING_FRONT)
                rotateImage(_picture, 270f)!!
            else
                rotateImage(_picture, 90f)!!
        ...
        val result = keepIndexes.map {
            ...
            if ((xmin >= xmax) || ymin >= ymax) {
                ...
                val imageBB = bb.getSquare(
                    width.toDouble(),
                    height.toDouble(),
                    0.5
                )  // Ensure it's square for prettier display
                val face = frame.submat(
                    Rect(
                        Point(imageBB.left, imageBB.top),
                ...
                val flippedFacePic =
                    Bitmap.createBitmap(facePic, 0, 0, faceWidth, faceHeight, matrix, true)
    ...
    private fun singleClassNonMaskSuppression(
        boundingBoxes: Mat,
        confidences: Mat,
        ...
        for (i in 0 until idxs.size) {
            tmp.put(i, 0, xMin[idxs[i], 0][0])
        }
        tmp.copyTo(xMin)
        for (i in 0 until idxs.size) {
            tmp.put(i, 0, xMax[idxs[i], 0][0])
        }
        tmp.copyTo(xMax)
        for (i in 0 until idxs.size) {
            tmp.put(i, 0, yMin[idxs[i], 0][0])
        }
        tmp.copyTo(yMin)
        for (i in 0 until idxs.size) {
            tmp.put(i, 0, yMax[idxs[i], 0][0])
        }
        tmp.copyTo(yMax)
        for (i in 0 until idxs.size) {
            tmp.put(i, 0, area[idxs[i], 0][0])
        }
        tmp.copyTo(area)
